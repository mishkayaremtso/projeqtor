...
class ProductAsset extends SqlElement {
  ...
  private static $_layout='
    ...
    <th field="nameAsset" width="40%" >${idAsset}</th>
  ...
  public function control() {
  	$result="";
  	if (! $this->id) {
  	  $crit=array('idAsset'=>$this->idAsset, 'idProductVersion'=>$this->idProductVersion);
  	  $list=$this->getSqlElementsFromCriteria($crit, false,null,null,null,true);
  	  if (count($list)>0) {
        $result.='<br/>' . i18n('errorDuplicateProductAsset');
      }     
  	}
